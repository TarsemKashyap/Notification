...
{
    public class TPRecurringCardPlanCreatedEventHandler : IEventHandler<TPRecurringCardPlanCreatedEvent>
    {
        #region Properties and local variables

        ILog _logger = LogManager.GetLogger(typeof(TPRecurringCardPlanCreatedEventHandler));
        ...
        public TPRecurringCardPlanCreatedEventHandler(
        ...
        public void HandleEvent(TPRecurringCardPlanCreatedEvent eventData)
        {
            using (_logger.Push())
            {
                _logger.Info("Handling event: TPRecurringCardPlanCreatedEvent", eventData);
                ...
                using (var uow = _storage.NewUnitOfWork())
                {
                    ...
                    Event eventModel = new Event(EventType.RpCardPlanCreated, DateTime.UtcNow, tpRecurringCardPlan.Merchant.Id, jsonString, "TPRecurringCardPlanCreatedEventHandler", ContentType.CardPlan,secret);
