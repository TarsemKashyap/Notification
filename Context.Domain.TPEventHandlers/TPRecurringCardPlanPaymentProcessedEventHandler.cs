...
{
    public class TPRecurringCardPlanPaymentProcessedEventHandler : IEventHandler<TPRecurringCardPlanPaymentProcessedEvent>
    {
        #region Properties and local variables

        ILog _logger = LogManager.GetLogger(typeof(TPRecurringCardPlanPaymentProcessedEventHandler));
        ...
        public TPRecurringCardPlanPaymentProcessedEventHandler(
        ...
        public void HandleEvent(TPRecurringCardPlanPaymentProcessedEvent eventData)
        {
            using (_logger.Push())
            {
                _logger.Info("Handling event: TPRecurringCardPlanPaymentProcessedEvent", eventData);
                ...
                using (var uow = _storage.NewUnitOfWork())
                {
                    ...
                    Event eventModel = new Event(EventType.RpCardPlanPaymentProcessed, DateTime.UtcNow, tpRecurringCardPlan.Merchant.Id, jsonString, "TPRecurringCardPlanPaymentProcessedEventHandler", ContentType.CardPlan,secret);
