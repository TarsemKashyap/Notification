...
{
    public class TPRecurringDdPlanCreatedEventHandler : IEventHandler<TPRecurringDdPlanCreatedEvent>
    {
        #region Properties and local variables

        ILog _logger = LogManager.GetLogger(typeof(TPRecurringDdPlanCreatedEventHandler));
        ...
        public TPRecurringDdPlanCreatedEventHandler(
        ...
        public void HandleEvent(TPRecurringDdPlanCreatedEvent eventData)
        {
            using (_logger.Push())
            {
                _logger.Info("Handling event: TPRecurringDdPlanCreatedEvent", eventData);
                ...
                using (var uow = _storage.NewUnitOfWork())
                {
                    ...
                    Event eventModel = new Event(EventType.RpDdPlanCreated, DateTime.UtcNow, tpRecurringDDPlan.Merchant.Id, jsonString, "TPRecurringDdPlanCreatedEventHandler", ContentType.DirectDebitPlan,secret);
