...
{
    public class TPRecurringDdPlanStatusChangedEventHandler : IEventHandler<TPRecurringDdPlanStatusChangedEvent>
    {
        #region Properties and local variables

        ILog _logger = LogManager.GetLogger(typeof(TPRecurringDdPlanStatusChangedEventHandler));
        ...
        public TPRecurringDdPlanStatusChangedEventHandler(
        ...
        public void HandleEvent(TPRecurringDdPlanStatusChangedEvent eventData)
        {
            using (_logger.Push())
            {
                _logger.Info("Handling event: TPRecurringDdPlanStatusChangedEvent", eventData);
                ...
                var prevoiusPlanStatus = (DirectDebitPlanStatus)eventData.OldStatusId;

                var currentPlanStatus = (DirectDebitPlanStatus)tpRecurringDDPlan.Status;

                try
                {
                    EventType eventType = GetEventType(prevoiusPlanStatus, currentPlanStatus);
                    ...
                    using (var uow = _storage.NewUnitOfWork())
                    {
                        ...
                        Event eventModel = new Event(eventType, DateTime.UtcNow, tpRecurringDDPlan.Merchant.Id, jsonString, "TPRecurringDdPlanStatusChangedEventHandler", ContentType.DirectDebitPlan,secret);
        ...
        private EventType GetEventType(DirectDebitPlanStatus ddPreviousStatus, DirectDebitPlanStatus ddCurrentStatus)
        {
            if (ddCurrentStatus == DirectDebitPlanStatus.Active && (ddPreviousStatus == DirectDebitPlanStatus.PendingApproval || ddPreviousStatus == DirectDebitPlanStatus.PendingVoiceVerification || ddPreviousStatus == DirectDebitPlanStatus.PendingEmailVerification))
            {
                return EventType.RpDdPlanApproved;
            }
            else if (ddCurrentStatus == DirectDebitPlanStatus.Suspended || (ddCurrentStatus == DirectDebitPlanStatus.Suspended && ddPreviousStatus == DirectDebitPlanStatus.SuspendingNewAuthority))
            {
                return EventType.RpDdPlanSuspended;
            }
            else if ((ddCurrentStatus == DirectDebitPlanStatus.Active && ddPreviousStatus == DirectDebitPlanStatus.Suspended) || (ddCurrentStatus == DirectDebitPlanStatus.Active && ddPreviousStatus == DirectDebitPlanStatus.SuspendingNewAuthority) || (ddCurrentStatus == DirectDebitPlanStatus.ActiveWithAmendment && ddPreviousStatus == DirectDebitPlanStatus.Suspended) || (ddCurrentStatus == DirectDebitPlanStatus.ActiveWithAmendment && ddPreviousStatus == DirectDebitPlanStatus.SuspendingNewAuthority) || (ddCurrentStatus == DirectDebitPlanStatus.ActiveWithDishonor && ddPreviousStatus == DirectDebitPlanStatus.Suspended) || (ddCurrentStatus == DirectDebitPlanStatus.ActiveWithDishonor && ddPreviousStatus == DirectDebitPlanStatus.SuspendingNewAuthority))
            {
                return EventType.RpDdPlanResumed;
            }
            else if (ddCurrentStatus == DirectDebitPlanStatus.Ended || (ddCurrentStatus == DirectDebitPlanStatus.EndedDishonor))
            {
                return EventType.RpDdPlanEnded;
            }
            else if (ddCurrentStatus == DirectDebitPlanStatus.Cancelled)
            {
                return EventType.RpDdPlanCancelled;
            }
            else if (ddCurrentStatus == DirectDebitPlanStatus.Rejected)
            {
                return EventType.RpDdPlanRejected;
            }
            else
            {
                throw new EventTypeNotFoundException(String.Format("Event type not found for status {0}", ddCurrentStatus.ToString()));
            }
        }
    }
}
